...
/** FR-04's GRN Flow 1 — Direct Entry (no PO). Manual supplier + line-item
 * entry; no ordered-quantity variance check applies (nothing was ordered). */
export function DirectGrnForm() {
  ...
  const [lines, setLines] = useState<GrnLineInput[]>([{ itemId: '', receivedQty: '', actualPrice: '' }]);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <h1 className="font-display text-xl font-semibold text-foreground">{t('grn.new.direct.title')}</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-surface p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Field label={t('grn.form.invoiceNumber')}>
              <CompactInput value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} />
            </Field>
